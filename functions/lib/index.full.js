...
exports.clickRedirect = exports.openPixel = exports.sendTestEmailGet = exports.scheduledBudget = exports.refreshFxRates = exports.setUserLocale = exports.sendTestEmail = exports.isFxAdmin = exports.setFxRates = exports.sendTestPush = exports.initUserProfile = exports.registerToken = exports.pingCallable = exports.ping = exports.health = void 0;
...
exports.registerToken = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    ...
    if (!userId) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to register a token');
    }
...
exports.initUserProfile = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    ...
    if (!uid) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to initialise user profile');
    }
...
exports.sendTestPush = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    const userId = request.auth?.uid ?? (typeof request.data?.userId === 'string' ? request.data.userId : '');
    if (!userId) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to send a test push notification');
    }
    const authEmail = typeof request.auth?.token?.email === 'string' ? request.auth.token.email : null;
    const authLocale = typeof request.auth?.token?.locale === 'string' ? request.auth.token.locale : null;
    await ensureUserProfile({ uid: userId, email: authEmail, locale: authLocale });
...
exports.setFxRates = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    if (!request.auth?.uid) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to update FX rates');
    }
...
exports.isFxAdmin = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    if (!request.auth?.uid) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to check admin status');
    }
...
exports.sendTestEmail = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    const userId = request.auth?.uid ?? (typeof request.data?.userId === 'string' ? request.data.userId : '');
    if (!userId) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to send a test email');
    }
    const authEmail = typeof request.auth?.token?.email === 'string' ? request.auth.token.email : null;
    const authLocale = typeof request.auth?.token?.locale === 'string' ? request.auth.token.locale : null;
    await ensureUserProfile({ uid: userId, email: authEmail, locale: authLocale });
...
exports.setUserLocale = (0, https_2.onCall)(HTTPS_OPTIONS, async (request) => {
    ...
    if (!userId) {
        throw new https_2.HttpsError('failed-precondition', 'Authentication is required to update locale');
    }
    const localeInput = typeof request.data?.locale === 'string' ? request.data.locale.trim() : '';
    if (!localeInput) {
