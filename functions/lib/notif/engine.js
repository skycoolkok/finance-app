...
class NotificationEngine {
  ...
  async deliverReminder(reminder) {
    ...
    const abVariant = (0, abtest_1.assignVariant)({
      userId: reminder.userId,
  ...
  async deliverEmail(reminder, content, locale, abVariant) {
    ...
    const emailContext = {
      ...
      preferencesUrl:
        content.email.context?.preferencesUrl ??
        this.appendPath(this.baseUrl, '/settings/notifications'),
      logoUrl:
        content.email.context?.logoUrl ?? this.appendPath(this.baseUrl, '/icons/icon-192.png'),
    }
    let html
    try {
